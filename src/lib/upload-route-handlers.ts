...
export function createUploadRouteHandlers(statePathname: string, uploadPrefix: string) {
  async function POST(request: Request) {
    ...
    const pathname = `${uploadPrefix}${taskId}/${Date.now()}-${sanitizeFilename(file.name)}`;
  ...
  async function DELETE(request: Request) {
    ...
    if (
      ...
      !pathname.startsWith(`${uploadPrefix}${taskId}/`)
    ) {
      return NextResponse.json({ error: "invalid request" }, { status: 400 });
    }
